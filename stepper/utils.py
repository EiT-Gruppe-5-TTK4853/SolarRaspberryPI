...
class Stepper:
    def __init__(self, out_pin, dir_pin, pos=0, ms=1, vel=1, gear_ratio=1.0) -> None:
        """
        out_pin and dir_pin must be of type LED from gpiozero. 
        """
        assert(ms in [1, 2, 4, 8, 16, 32]), f"Invalid microstepping. Needs to be one of [1, 2, 4, 8, 16, 32], got {ms}"
        ...
        self.da = self.gear_ratio * 360 / self.steps_per_rotation 
    ...
    def move(self, steps, dir=1, vel=None):
        ...
        if vel == None:
            ...
            assert(abs(vel) < 10), f"You are going too fast, the velocity is capped at 10 rot/s. Got {vel} rot/s"
        ...
        print(f"Took {i+1} steps in {(time_ns() - t0) / 1000000} ms")
    ...
    def move_to(self, angle, pos=None, vel=None, rad=False):
        ...
        print(f"Moving to angle {angle} from {self.pos}. This takes {steps} steps in {dir} direction")
